...
extern (C++) Expression resolveUFCS(Scope* sc, CallExp ce)
{
    ...
    if (ce.e1.op == TOK.dotIdentifier)
    {
        ...
        if (t.ty == Type.Kind.array || t.ty == Type.Kind.staticArray || t.ty == Tnull || (t.isTypeBasic() && t.ty != Type.Kind.void_))
        {
...
extern (C++) final class ArrayLiteralExp : Expression
{
    ...
    override StringExp toStringExp()
    {
        ...
        if (telem == Tchar || telem == Twchar || telem == Tdchar || (telem == Type.Kind.void_ && (!elements || elements.dim == 0)))
        {
            ubyte sz = 1;
            if (telem == Twchar)
                sz = 2;
