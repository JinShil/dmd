...
extern (C++) final class TypeAArray : TypeArray
{
    ...
    override void resolve(Loc loc, Scope* sc, Expression* pe, Type* pt, Dsymbol* ps, bool intypeid = false)
    {
        ...
        if (index.ty == Type.Kind.identifier || index.ty == Tinstance || index.ty == Type.Kind.staticArray)
        {
            Expression e;
            Type t;
            Dsymbol s;
...
extern (C++) final class TypeNull : Type
{
    ...
    override MATCH implicitConvTo(Type to)
    {
        ...
        //if (type.ty == Type.Kind.pointer && type.nextOf().ty == Type.Kind.void_)
        {
            ...
            if (tb.ty == Tnull || tb.ty == Type.Kind.pointer || tb.ty == Type.Kind.array || tb.ty == Type.Kind.associativeArray || tb.ty == Type.Kind.class_ || tb.ty == Type.Kind.delegate_)
                return MATCH.constant;
        }
